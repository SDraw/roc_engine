...
{
    internal static class CylinderColliderDefs
    {
        ...
        static CylinderColliderDefs()
        {
            Definition.m_name = nameof(CylinderCollider);
        ...
        static int GetSize(IntPtr p_state)
        {
            ...
            if(!l_argReader.ReadObject(out CylinderCollider l_col))
            {
        ...
        {
            var l_argReader = new ArgReader(p_state);
            if(!l_argReader.ReadObject(out CylinderCollider l_col) || !l_argReader.ReadObject(out Vector3 l_val))
                return 0;
