...
{
    internal static class CapsuleColliderDefs
    {
        public static readonly LuaVM.LuaClassDefinition Definition = new LuaVM.LuaClassDefinition();

        static CapsuleColliderDefs()
        {
            Definition.m_name = nameof(CapsuleCollider);
        ...
        static int GetRadius(IntPtr p_state)
        {
            ...
            if(!l_argReader.ReadObject(out CapsuleCollider l_col))
            {
        ...
        {
            var l_argReader = new ArgReader(p_state);
            if(!l_argReader.ReadObject(out CapsuleCollider l_col) || !l_argReader.ReadNumber(out float l_val))
                return 0;
