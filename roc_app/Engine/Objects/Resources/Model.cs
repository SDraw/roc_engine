        ...
        internal void Load(string p_path)
        {
            ...
            try
            {
                ...
                for(uint i = 0; i < l_materialsCount; i++)
                {
                    ...
                    l_subMesh.m_materialColor.r = l_reader.ReadSingle();
                    l_subMesh.m_materialColor.g = l_reader.ReadSingle();
                    l_subMesh.m_materialColor.b = l_reader.ReadSingle();
                    l_subMesh.m_materialColor.a = l_reader.ReadSingle();
                    ...
                    l_subMesh.m_texturePath = Encoding.UTF8.GetString(l_reader.ReadBytes(l_textureNameLength));

                    // Vertex
                    int l_dataSize = l_reader.ReadInt32();
                    {
                        byte[] l_data = l_reader.ReadBytes(l_dataSize);
                        l_subMesh.m_vertexData = new float[l_dataSize / 4];
                        Buffer.BlockCopy(l_data, 0, l_subMesh.m_vertexData, 0, l_dataSize);
                    }
                    ...
                    {
                        byte[] l_data = l_reader.ReadBytes(l_dataSize);
                        l_subMesh.m_normalData = new float[l_dataSize / 4];
                        Buffer.BlockCopy(l_data, 0, l_subMesh.m_normalData, 0, l_dataSize);
                    }
                    ...
                    {
                        byte[] l_data = l_reader.ReadBytes(l_dataSize);
                        l_subMesh.m_uvData = new float[l_dataSize / 4];
                    ...
                    if(m_modelType == (byte)ModelType.Rigged)
                    {
                        ...
                        {
                            byte[] l_data = l_reader.ReadBytes(l_dataSize);
                            l_subMesh.m_weightData = new float[l_dataSize / 4];
                            Buffer.BlockCopy(l_data, 0, l_subMesh.m_weightData, 0, l_dataSize);
                        }
                        ...
                        {
                            byte[] l_data = l_reader.ReadBytes(l_dataSize);
                            l_subMesh.m_indexData = new int[l_dataSize / 4];
                            Buffer.BlockCopy(l_data, 0, l_subMesh.m_indexData, 0, l_dataSize);
                        }
                    }
                    else
                    {
                ...
                if(m_modelType == (byte)ModelType.Rigged)
                {
                    ...
                    for(uint i = 0U; i < l_bonesCount; i++)
                    {
                        ...
                        l_boneData.m_name = Encoding.UTF8.GetString(l_reader.ReadBytes(l_boneNameLength));
